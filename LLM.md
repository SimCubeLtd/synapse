...
synapse status --json   # {index:"ready"|"empty", filesIndexed, symbolsIndexed, ...}
```
...
#   (IMPLEMENTS), subtype/implementor (reverse), same directory, likely tests.
...
INHERITS, IMPLEMENTS (symbol→symbol).
Edges (schema only, empty): CALLS, REFERENCES — no call-graph yet; don't rely on them.
